...
# get 1/0 on raster grid of presence 
...
# for(i in 138:length(gbifs_in_folder)){
  ...
  if(nrow(occ_sub) > 0){
    ...
    # writeRaster(presence_raster, 
    #             filename=file.path(glue("data/occurences/occ_raster/presence/{gbifId}.tif")), 
  ...
  rm(gbifId, occ, pts_clean_rm, pts_oor, count_raster, 
     pts_oor_rm, occ_sub, occ_sub_sp, m, rclmat,
     presence_raster)
